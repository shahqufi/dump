...
tran_ota_dir="/tranfs/MTK_OTA_Gen97"
tranfs_dir="/tranfs"
tranfs_sim_status_0=""
tranfs_sim_status_1=""
...
if [ $1 -gt 0 ] ; then
        ...
        if [ ${runtime_dir_size} -lt 100 ] ; then
                echo -e "Runtime_dir_size check PASS"
                # move new mcfota files and change proper permission
                cp -pf "${tran_ota_dir}"/*.mcfopota "${mcf_runtime_dir}"
                if [ $? -ne 0 ] ; then
                        echo -e "Move MCF file failed"
                        _exit 1
                fi

                chmod -R 644 "${mcf_runtime_dir}"/*.mcfopota
                if [ $? -ne 0 ] ; then
                        echo -e "No permission to control MCF files"
                        _exit 1
                fi
        else
                echo -e "Runtime_dir_size must be less than 100MB"
                _exit 1
        fi
else
...
if [ $tranfs_sim_status_1 == "1" ] ; then 
        ...
        echo -e "Get PS2 GENERAL _BIN"
        echo -e at+emcfc=4,2\\r > /dev/pts/21
        #check at respond
        get_carrier_mcf_at_respond
fi
}
...
if [ $1 -eq 1 ] && [ $2 == "${tran_ota_dir}/MTK_OPOTA_GENERAL.mcfopota" ] ; then 
        #GENERAL_BIN
        echo -e "reset GENERAL_BIN to auto select mode"
        echo -e at+emcfc=6,2,1,\"\",1\\r > /dev/pts/7
        #check at respond
        check_auto_mode_mcf_at_respond
        check_auto_mode_dsbp_at_respond
else 
        #CARRIER_BIN
        echo -e "reset CARRIER_BIN to auto select mode"
        echo -e at+emcfc=6,1,1,\"\",1\\r > /dev/pts/7
        #check at respond
        check_auto_mode_mcf_at_respond
        check_auto_mode_dsbp_at_respond
fi
...
if echo -e ${tranfs_platform_support}|grep -iq "${TRAN_MTK_PLATFORM_GEN97}" ;then
        ...
        ota_file_num=$(ls ${tran_ota_dir}/*.mcfopota |wc -l) 2>/dev/null
        ota_file=$(ls ${tran_ota_dir}/*.mcfopota) 2>/dev/null
        move_to_runtime_dir ${ota_file_num} 

        echo -e "####    Step 3:CHECK SIM STATUS       #### "
        check_sim_card_status

        echo -e "####    Step 4:SET AUTO SELECT MODE TO MD   ####"
        set_auto_select_mode ${ota_file_num} ${ota_file} 
...
if [ $tranfs_sim_status_0 == "0" ] && [ $tranfs_sim_status_1 == "0" ] ;then 
